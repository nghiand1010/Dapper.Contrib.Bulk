...
namespace Dapper.Tests.Contrib.Bulk
{
    ...
    public class PosgreSQLTestSuite : TestSuite
    {
        public static string ConnectionString { get; } =
            GetConnectionString("PosgreeSQLConnectionString", "Server=localhost;Port=5432;Database=test;Username=postgres;Password=postgres");

        public override IDbConnection GetConnection()
        {


            if (_skip) Skip.Inconclusive("Skipping PosgreSQLTestSuite Tests - no server.");
            return new NpgsqlConnection(ConnectionString);
        }

        private static readonly bool _skip;

        static PosgreSQLTestSuite()
        {
            try
            {
                using (var connection = new NpgsqlConnection(ConnectionString))
                {
                    ...
                    connection.Open();
                    dropTable("Stuff");
                    connection.Execute("CREATE TABLE Stuff (Id   serial primary key, name text not null, created timestamp null);");
                    dropTable("People");
                    ...
                    dropTable("ObjectX");
                    connection.Execute("CREATE TABLE ObjectX (ObjectX_Id text not null, Name text not null);");
                    dropTable("ObjectY");
                    connection.Execute("CREATE TABLE ObjectY (ObjectY_Id int not null, Name text not null);");
                    dropTable("ObjectZ");
                    ...
                    dropTable("NullableDates");
                    connection.Execute("CREATE TABLE NullableDates (Id   serial PRIMARY KEY, Date_Value timestamp);");
                }
            }
